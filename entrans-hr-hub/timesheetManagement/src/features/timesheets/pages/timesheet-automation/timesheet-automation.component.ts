...
export class TimesheetAutomationComponent {
  ...
  pageSizeDetails: number = 10;

  get totalPagesDetails(): number {
    const dataLen = this.validatedData[this.selectedUserName]?.length || 0;
    return Math.max(1, Math.ceil(dataLen / this.pageSizeDetails));
  }

  get paginatedDetails(): any[] {
    const data = this.validatedData[this.selectedUserName] || [];
    const start = (this.currentPageDetails - 1) * this.pageSizeDetails;
    return data.slice(start, start + this.pageSizeDetails);
  }

  get pageNumbersDetails(): number[] {
    const total = this.totalPagesDetails;
    const cur = this.currentPageDetails;
  ...
  goToPageDetails(page: number) {
    if (page >= 1 && page <= this.totalPagesDetails) {
  ...
  pageSizeSummary: number = 10;

  get totalPagesSummary(): number {
    const dataLen = this.validationSummary?.length || 0;
    return Math.max(1, Math.ceil(dataLen / this.pageSizeSummary));
  }

  get paginatedSummary(): any[] {
    const data = this.validationSummary || [];
    const start = (this.currentPageSummary - 1) * this.pageSizeSummary;
    return data.slice(start, start + this.pageSizeSummary);
  }

  get pageNumbersSummary(): number[] {
    const total = this.totalPagesSummary;
    const cur = this.currentPageSummary;
    if (total <= 7) return Array.from({ length: total }, (_, i) => i + 1);
    const pages: number[] = [];
    for (let i = 1; i <= total; i++) {
      if (i === 1 || i === total || (i >= cur - 2 && i <= cur + 2)) {
  ...
  goToPageSummary(page: number) {
    if (page >= 1 && page <= this.totalPagesSummary) {
